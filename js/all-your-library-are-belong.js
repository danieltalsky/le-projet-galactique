...
class Viewport extends ScreenComponent {
  ...
  makeNewLocationAppear() {
    this.contentElement.style.backgroundSize = 'contain';    
    this.contentElement.style.backgroundImage = 'url(/img/sample-location.png)';
    this.contentElement.style.backgroundRepeat = 'no-repeat';
    this.contentElement.style.backgroundPositionX = '0';
  }
}
...
class Readout extends ScreenComponent {
  ...
  displayLocationText() {
    ...
    const possibilities = Object.keys(this.possibleThingsOnAPlanet);
    const categoryFound = possibilities[Math.floor(Math.random() * possibilities.length)];
    const sourceText = this.possibleThingsOnAPlanet[categoryFound];
    const additionalDetails = this.additionalDetails[categoryFound];
    let displayText = sourceText.replace('{1}', additionalDetails[Math.floor(Math.random() * additionalDetails.length)]);
    displayText = displayText.replace('{2}', additionalDetails[Math.floor(Math.random() * additionalDetails.length)]);
